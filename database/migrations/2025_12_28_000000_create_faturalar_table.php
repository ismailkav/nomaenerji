...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('faturalar', function (Blueprint $table) {
            $table->id();
            $table->string('siparis_turu', 20)->default('alim'); // alim | satis | alim-iade | satis-iade
            ...
            $table->string('teklif_no', 50)->nullable();
            $table->index('teklif_no');

            $table->text('aciklama')->nullable();
            $table->string('siparis_durum', 50)->nullable();

            $table->string('planlama_durum', 50)->nullable();
            $table->decimal('planlanan_miktar', 15, 2)->nullable();
